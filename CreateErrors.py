    ...
    for sentence in sentence_list:
        ...
        if word_count_to_select != 0:
            ...
            for word_ind in rand_word_indices:
                ...
                word = words[word_ind]
                if min_error_index in [2,3,4]:
                    #wordün keyboard dictte charı varsa onu seç
                    if any(x in keyboard_closest_chars.keys() for x in list(word)):
                        for q in range(len(word)):
                            if word[q] in keyboard_closest_chars.keys():
                                rand_error_position = q
                    #yoksa başka hata seç
                    else:
                        min_error_index = random.choice([0,1,5,6,7])
                match min_error_index:
                    ...
                    case 2: #change_chars_prev
                        if word[rand_error_position] in keyboard_closest_chars.keys():
